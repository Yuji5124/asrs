...
## 2026-07-04 マップ制作画面のUI磨き込み

- パレットを「地形タイル / 配置物 / マップ設定」の3セクションに分類し、キャンバスと同じ色・記号のスウォッチ、通行不可バッジ、下部の「選択中のツール」表示を追加。UI状態のみの変更で保存スキーマは不変。
- キャンバスにホバー中マスの枠ハイライトと、下部の情報バー（座標・タイル種・通行可否・配置物名・マップサイズ・配置物数）を追加。ホバー状態はMapCanvasのローカルstateで持ち、storeとスキーマには入れない。
- カーソル形状をツール連動に（塗り=crosshair / 配置=copy / 配置物上=pointer）。ツールバーに「マップ編集」モードチップを追加し、狭い画面ではツールバー折り返し＋左右パネル幅縮小（@media 1080px）。
- ブラシサイズ・塗りつぶし・矩形描画・undo/redo・スポイト・ミニマップ/ズームは今回実装しない（候補として整理のみ。導入時はundo/redoを先に入れるべき）。

## 2026-07-04 マップ制作画面の追加磨き込み

- ホバー中のマスに選択中ツールの半透明プレビューを表示する。既存配置物の上では配置プレビューを出さず、クリックで配置物選択になることを優先する。
- Canvas下の情報バーにクリック時の操作文を追加する。これは制作補助のUI表示のみで、storeや保存JSONのスキーマは増やさない。
- 760px以下ではパレット・Canvas・設定パネルを縦に積む。狭い画面では編集効率より破綻しない表示を優先する。

## 2026-07-04 マップ編集ツール強化（ペン/塗りつぶし/矩形・ズーム・undo/redo）

- コンセプト画像を参考に、機能要素のみ取り込む: タイル編集ツール（ペン / 塗りつぶし / 矩形）、ズーム（50/75/100/150/200%）、元に戻す・やり直す。ピクセルアート素材・タイル種追加・ミニマップは取り込まない（素材なし方針とスキーマ不変を維持）。
- 塗りつぶし（連結領域のflood fill）と矩形塗りは `src/core/map/paint.ts` の純関数として実装し、ユニットテストを付ける。UIはそれを呼ぶだけ。
- undo/redoはプロジェクト全体のスナップショット方式（最大50件、メモリ内のみ・保存JSONには含めない）。ドラッグ1ストローク=undo1回にまとめる。名前・メッセージのテキスト編集は1文字ごとに履歴が汚れるため履歴対象外とする。
- ズームはCanvasのCSS表示サイズのみ変更（内部解像度は不変）。クリック座標は表示幅に対する比率で算出し、倍率に依存しない。
